...
router.get('/get-client-partner-status', authenticate, async (req, res) => {
  try {
    const client_id = req.user.id;
    const query = "SELECT isSubmittedPartnerCust FROM Customer WHERE idCust = ?";
    ...
    db.query(query, [client_id], (err, result) => {
      ...
      if (result.length > 0) {
        const isSubmittedPartnerCust = result[0].isSubmittedPartnerCust;
        res.status(200).json({ isSubmittedPartnerCust });
      } else {
